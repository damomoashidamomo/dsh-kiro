...
export function mapKey(event: RawKey): KeyAction | undefined {
  const { input, key } = event

  // Modifier-prefixed shortcuts.
  if (key.ctrl) {
    if (input === 'c') return 'interrupt'
    if (input === 'd') return 'quit'
    if (input === 'k') return 'open-slash-menu'
    if (input === 't') return 'toggle-tangent'
    if (input === 'l') return 'clear-screen'
    if (input === 'g') return 'cancel-pending'
    if (input === 'p') return 'history-prev'
    if (input === 'n') return 'history-next'
    if (input === 'r') return 'history-prev'
    if (input === 'o') return 'open-context-picker'
    return undefined
  }
  if (key.meta) {
    if (input === 'k') return 'open-slash-menu'
  ...
  if (key.shift) {
    if (key.tab) return 'toggle-plan'
    if (input === 'Tab') return 'toggle-plan'
    return undefined
  }
  // Special keys without modifier.
  if (key.escape) return 'cancel-pending'
  if (key.return) return 'submit'
  if (key.upArrow) return 'history-prev'
  if (key.downArrow) return 'history-next'
  if (key.tab) return 'auto-complete'
  if (key.pageUp) return 'scroll-up'
  if (key.pageDown) return 'scroll-down'
  if (key.leftArrow || key.rightArrow) return undefined
  return undefined
}

/** Default chord display for documentation purposes. */
export const CHEAT_SHEET: ReadonlyArray<readonly [string, KeyAction, string]> = [
  ['Enter', 'submit', '提交输入'],
