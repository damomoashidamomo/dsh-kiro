...
export type InputOutcome =
  | { kind: 'ignore' }
  | { kind: 'submit'; text: string; images: readonly PendingImage[] }
  | { kind: 'history-prev' }
  | { kind: 'history-next' }
  | { kind: 'auto-complete' }
  | { kind: 'newline' }
  | { kind: 'special-prefix'; prefix: '/' | '@' | '!' }
  | { kind: 'backspace' }
  | { kind: 'delete' }
  | { kind: 'cursor-left'; word: boolean }
  | { kind: 'cursor-right'; word: boolean }
  | { kind: 'cursor-home' }
  | { kind: 'cursor-end' }
...
export function emptyBuffer(history: readonly string[] = []): PromptBuffer {
  return {
    text: '',
    cursor: 0,
    history,
...
export function applyOutcome(
  ...
  switch (outcome.kind) {
    ...
    case 'submit': {
      ...
      if (text === '' && buffer.images.length === 0) return buffer
...
function stepHistory(buffer: PromptBuffer, delta: 1 | -1): PromptBuffer {
  const { history, historyIndex } = buffer
  if (history.length === 0) return buffer
  const next = historyIndex + delta
  if (next < -1 || next >= history.length) return buffer
  if (next === -1) {
