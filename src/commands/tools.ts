...
function resolveStore(ctx: Context): TrustStore | undefined {
  return ctx.get(KIRO_TRUST)
}

function formatList(store: TrustStore | undefined): string {
  ...
  if (trustAll) lines.push('  trust-all is on (every tool runs without confirmation)')
  return lines.join('\n')
}
...
async function runToolsCommand(sub: string, args: string[]): Promise<{ kind: 'success'; text: string }> {
  ...
  switch (sub) {
    ...
    case 'trust': {
      const tool = args[0]
      if (tool === undefined) return { kind: 'success', text: 'usage: /tools trust <name>' }
      const next = {
        ...doc,
    ...
    case 'untrust': {
      const tool = args[0]
      if (tool === undefined) return { kind: 'success', text: 'usage: /tools untrust <name>' }
...
function formatListFromDoc(doc: { trustAll: boolean; entries: Record<string, unknown> }): string {
