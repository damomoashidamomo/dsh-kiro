...
function parseSubcommand(input: string): { name: string; args: string[] } {
  ...
  if (trimmed === '') return { name: 'show', args: [] }
  const tokens = trimmed.split(/\s+/u)
  return { name: tokens[0] ?? 'show', args: tokens.slice(1) }
}

function inspectPath(arg: string): string {
  const abs = resolve(arg)
  if (!existsSync(abs)) return `  ✗ ${arg}  (not found)`
  const stat = statSync(abs)
  if (stat.isDirectory()) return `  ▸ ${arg}  (directory, ${stat.size.toLocaleString()} bytes metadata)`
...
export const contextCommand: CommandDefinition = {
  ...
  handler: async ({ rawInput }) => {
    ...
    switch (sub) {
      case 'add': {
        const target = args[0]
        if (target === undefined) return { kind: 'success', text: 'usage: /context add <path>' }
