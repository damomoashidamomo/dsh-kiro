...
function wrapAnsi(text, width = MAX_WIDTH) {
    if (text === '')
        return [''];
    const lines = [];
    const tokens = [];
    ...
    while ((match = re.exec(text)) !== null) {
        if (match[1] !== undefined)
            tokens.push({ ansi: match[1], plain: '' });
        else if (match[2] !== undefined)
            tokens.push({ ansi: '', plain: match[2] });
    }
    let plainBuf = '';
    let pendingAnsi = '';
    let visibleLen = 0;
    const flush = () => {
        if (plainBuf === '' && pendingAnsi === '')
            return;
...
function renderCode(token) {
    ...
    const visibleWidth = Math.min(MAX_WIDTH - 4, Math.max(...lines.map(line => stripAnsi(line).length), 1));
...
function renderTable(token) {
    ...
    const widths = headerCells.map((cell, i) => Math.max(stripAnsi(cell).length, ...token.rows.map(row => {
        const c = row[i];
    ...
    const sepCells = widths.map((width, i) => {
        ...
        if (align === 'right')
            return `${palette.dim(':')}${bar.slice(1)}${palette.dim(':')}`;
        if (align === 'center')
            return `${palette.dim(':')}${bar.slice(1, -1)}${palette.dim(':')}`;
    ...
    const bodyLines = token.rows.map(row => palette.dim('│ ') + row.map((cell, i) => pad(renderInline(cell.tokens), widths[i] ?? 0, alignments[i] ?? 'left')).join(palette.dim(' │ ')) + palette.dim(' │'));
...
export function renderInline_(source) {
    if (source === '')
        return '';
